...
func TestCatalogImplementationStatusIsExplicit(t *testing.T) {
	...
	for _, name := range []string{"list_runs", "get_run", "stream_run_chat", "check_ready", "get_config_status", "list_skills", "approve_run", "publish_final"} {
		if !byName[name].Implemented {
			t.Fatalf("%s should be marked implemented: %#v", name, byName[name])
		}
	}
	for _, name := range []string{"get_selected_context", "get_diff_summary", "list_provider_status", "get_publish_status", "revise_draft", "suppress_finding", "rerun_review", "preview_memory_proposal"} {
		if byName[name].Implemented {
